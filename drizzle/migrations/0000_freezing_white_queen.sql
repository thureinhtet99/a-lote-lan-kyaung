...
CREATE TYPE "public"."job_listing_location_requirement" AS ENUM('on-site', 'hybrid', 'remote');--> statement-breakpoint
...
CREATE TABLE "job_listings" (
	...
	"description" text,
	"wage" integer,
	"wageIntervel" "job_listing_wage_interval",
...
--> statement-breakpoint
CREATE TABLE "user_noti_settings" (
	"userId" varchar PRIMARY KEY NOT NULL,
	"newJobEmailNoti" boolean DEFAULT false NOT NULL,
	"aiPrompt" varchar,
	"createdAt" timestamp with time zone DEFAULT now() NOT NULL,
	"updatedAt" timestamp with time zone DEFAULT now() NOT NULL
);
...
CREATE TABLE "organization_user_settings" (
	...
	"newApplicationEmailNoti" boolean DEFAULT false NOT NULL,
...
ALTER TABLE "user_noti_settings" ADD CONSTRAINT "user_noti_settings_userId_users_id_fk" FOREIGN KEY ("userId") REFERENCES "public"."users"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
